...
namespace wlw::utils {
  ...
  std::unique_ptr<RawImage> ImageLoader::LoadImage(const std::string &file_path) {
    ...
    return std::make_unique<RawImage>(core::Vector2{.x = static_cast<float>(width), .y = static_cast<float>(height)},
                                      channels_count, data_vector);
  }
